...
mod tests {
    ...
    fn test_escapes_turbofish_and_call_site_bindings() {
        logwise_runtime::context::Context::reset("test_macro_hygiene".to_string());
        ...
        logwise_runtime::info_sync!("emoji \u{1F600} {key}", key = 42);

        // A comma inside a turbofish separates generic arguments, not macro arguments.
        logwise_runtime::info_sync!("len {len}", len = HashMap::<u8, u8>::new().len());
        ...
        let formatter = 2;
        let id = 3;
        logwise_runtime::info_sync!("hygiene {a}{b}{c}", a = record, b = formatter, c = id);
    ...
    fn test_perfwarn_block_sees_call_site_bindings() {
        ...
        let result = 32;
        let mut runs = 0;
        let value = logwise_runtime::perfwarn!("perfwarn hygiene", {
            runs += 1;
            interval + result
        });
        assert_eq!(value, 42);
