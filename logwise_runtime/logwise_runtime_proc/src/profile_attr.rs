...
//! The `#[logwise_runtime::profile]` attribute macro, the only entry point in this crate that
    ...
    let new_body_src = format!(
        r#"{{
            let _logwise_profile_guard = {{
                let (id, mut record) = logwise_runtime::hidden::profile_begin_pre(file!(), line!(), column!());
                let name = concat!(module_path!(), "::", "{fn_name}");
                record.log(name);
                logwise_runtime::hidden::profile_begin_post(id, record, name)
            }};
            {{ {original_body} }}
        }}"#,
        fn_name = fn_name,
