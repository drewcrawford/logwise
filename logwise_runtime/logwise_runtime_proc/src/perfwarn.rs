...
pub fn perfwarn_begin_impl(input: TokenStream) -> TokenStream {
    ...
    let src = format!(
        r#"
        {{
            if logwise_runtime::log_enabled!(logwise_runtime::Level::PerfWarn) {{
                let mut __logwise_record = logwise_runtime::hidden::perfwarn_begin_pre(file!(),line!(),column!());
                let mut __logwise_formatter = logwise_runtime::hidden::PrivateFormatter::new(&mut __logwise_record);
                {LFORMAT_EXPAND}
                logwise_runtime::hidden::perfwarn_begin_post(__logwise_record,"{NAME}")
            }} else {{
                logwise_runtime::hidden::perfwarn_begin_post(
                    logwise_runtime::hidden::perfwarn_begin_pre(file!(),line!(),column!()),
                    "{NAME}",
                )
            }}
        }}
...
pub fn perfwarn_impl(input: TokenStream) -> TokenStream {
    ...
    let src = format!(
        r#"
        {{
            let __logwise_interval = if logwise_runtime::log_enabled!(logwise_runtime::Level::PerfWarn) {{
                let mut __logwise_record = logwise_runtime::hidden::perfwarn_begin_pre(file!(),line!(),column!());
                let mut __logwise_formatter = logwise_runtime::hidden::PrivateFormatter::new(&mut __logwise_record);
                {LFORMAT_EXPAND}
                Some(logwise_runtime::hidden::perfwarn_begin_post(__logwise_record,"{NAME}"))
            }} else {{
                None
            }};
...
pub fn perfwarn_begin_if_impl(input: TokenStream) -> TokenStream {
    ...
    let src = format!(
        ...
        {{
            if logwise_runtime::log_enabled!(logwise_runtime::Level::PerfWarn) {{
                let mut __logwise_record = logwise_runtime::hidden::perfwarn_begin_if_pre(file!(),line!(),column!());
                let mut __logwise_formatter = logwise_runtime::hidden::PrivateFormatter::new(&mut __logwise_record);
                {LFORMAT_EXPAND}
                logwise_runtime::hidden::perfwarn_begin_if_post(__logwise_record, "{NAME}", {THRESHOLD})
            }} else {{
                logwise_runtime::hidden::perfwarn_begin_if_post(
                    logwise_runtime::hidden::perfwarn_begin_if_pre(file!(),line!(),column!()),
                    "{NAME}",
                    {THRESHOLD}
                )
            }}
