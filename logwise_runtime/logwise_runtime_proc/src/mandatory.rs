...
pub fn mandatory_sync_impl(input: TokenStream) -> TokenStream {
    ...
    let src = format!(
        r#"
        {{
            if logwise_runtime::log_enabled!(logwise_runtime::Level::Mandatory) {{
                let mut __logwise_record = logwise_runtime::hidden::mandatory_sync_pre(file!(),line!(),column!());

                let mut __logwise_formatter = logwise_runtime::hidden::PrivateFormatter::new(&mut __logwise_record);

                {LFORMAT_EXPAND}
                logwise_runtime::hidden::mandatory_sync_post(__logwise_record);
            }}
        }}
    "#,
    ...
    let src = format!(
        ...
        {{
            if logwise_runtime::log_enabled!(logwise_runtime::Level::Mandatory) {{
                ...
                {LFORMAT_EXPAND}
                logwise_runtime::hidden::mandatory_async_post(__logwise_record).await;
            }}
        }}
    "#,
        LFORMAT_EXPAND = lformat_result.output
