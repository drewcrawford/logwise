...
pub fn error_sync_impl(input: TokenStream) -> TokenStream {
    ...
    let src = format!(
        r#"
        {{
            if logwise_runtime::log_enabled!(logwise_runtime::Level::Error) {{
                let mut __logwise_record = logwise_runtime::hidden::error_sync_pre(file!(),line!(),column!());

                let mut __logwise_formatter = logwise_runtime::hidden::PrivateFormatter::new(&mut __logwise_record);

                {LFORMAT_EXPAND}
                logwise_runtime::hidden::error_sync_post(__logwise_record);
            }}
        }}
    "#,
    ...
    let src = format!(
        ...
        {{
            if logwise_runtime::log_enabled!(logwise_runtime::Level::Error) {{
                ...
                {LFORMAT_EXPAND}
                logwise_runtime::hidden::error_async_post(__logwise_record).await;
            }}
        }}
    "#,
        LFORMAT_EXPAND = lformat_result.output
