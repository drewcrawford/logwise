...
pub fn profile_begin_impl(input: TokenStream) -> TokenStream {
    ...
    let src = format!(
        r#"
        {{
            if logwise_runtime::log_enabled!(logwise_runtime::Level::Profile) {{
                let (__logwise_id, mut __logwise_record) = logwise_runtime::hidden::profile_begin_pre(file!(),line!(),column!());
                let mut __logwise_formatter = logwise_runtime::hidden::PrivateFormatter::new(&mut __logwise_record);
                {LFORMAT_EXPAND}
                logwise_runtime::hidden::profile_begin_post(__logwise_id, __logwise_record, "{NAME}")
            }} else {{
                let (__logwise_id, __logwise_record) = logwise_runtime::hidden::profile_begin_pre(file!(),line!(),column!());
                logwise_runtime::hidden::profile_begin_post(__logwise_id, __logwise_record, "{NAME}")
            }}
        }}
    "#,
...
pub fn profile_sync_impl(input: TokenStream) -> TokenStream {
    ...
    let src = format!(
        ...
        {{
            if logwise_runtime::log_enabled!(logwise_runtime::Level::Profile) {{
                let mut __logwise_record = logwise_runtime::hidden::profile_sync_pre(file!(),line!(),column!());

                let mut __logwise_formatter = logwise_runtime::hidden::PrivateFormatter::new(&mut __logwise_record);

                {LFORMAT_EXPAND}
                logwise_runtime::hidden::profile_sync_post(__logwise_record);
            }}
        }}
    "#,
        LFORMAT_EXPAND = lformat_result.output
    ...
    let src = format!(
        ...
        {{
            if logwise_runtime::log_enabled!(logwise_runtime::Level::Profile) {{
                ...
                {LFORMAT_EXPAND}
                logwise_runtime::hidden::profile_async_post(__logwise_record).await;
            }}
        }}
    "#,
        LFORMAT_EXPAND = lformat_result.output
