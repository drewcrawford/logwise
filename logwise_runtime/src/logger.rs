...
//! # fn main() {
//! use logwise_runtime::{Logger, add_global_logger};
//! use std::sync::Arc;
//!
//! # #[derive(Debug)]
//! # struct MyCustomLogger;
//! # impl Logger for MyCustomLogger {
//! #     fn finish_log_record(&self, _: logwise_runtime::LogRecord) {}
//! #     fn finish_log_record_async<'s>(&'s self, record: logwise_runtime::LogRecord)
//! #         -> std::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send + 's>> {
...
//! logwise_runtime::info_sync!("This message goes to all registered loggers");
//! # }
//! ```
//!
...
/// let mut record = LogRecord::new(logwise_runtime::Level::Info);
...
pub trait Logger: Debug + Send + Sync {
    ...
    ///
    /// ```
    /// use logwise_runtime::{Logger, LogRecord, Level};
    ///
    /// # #[derive(Debug)]
