...
/// # fn main() {
/// use logwise_runtime::context::{Context, ApplyContext};
///
/// async fn process_data() {
///     logwise_runtime::info_sync!("Processing data");
/// }
///
/// # async fn example() {
/// // Create a context for this operation
/// let ctx = Context::new_task(None, "data_processor".to_string(), logwise_runtime::Level::Info, true);
///
...
impl<F> ApplyContext<F> {
    ...
    ///     logwise_runtime::info_sync!("Running task");
    ///     42
    /// }
    ///
    /// # async fn example() {
    /// let ctx = Context::new_task(None, "wrapped_task".to_string(), logwise_runtime::Level::Info, true);
