...
fn main() {
    ...
    logwise_runtime::info_sync!("User logged in", user_id=42);

    // With multiple parameters
    logwise_runtime::warn_sync!("Request failed",
        status=404,
        path="/api/users"
    );
}
...
async fn handle_request() {
    logwise_runtime::info_async!("Processing request",
        method="GET",
        endpoint="/health"
    );
}
...
use logwise_runtime::privacy::{LogIt, IPromiseItsNotPrivate};
...
fn main() {
    ...
    logwise_runtime::info_sync!("User created", user=LogIt(&user));

    // Mark explicitly non-private data when it's safe
    let public_id = "PUBLIC-123";
    logwise_runtime::info_sync!("Processing {id}",
...
fn main() {
    ...
    logwise_runtime::perfwarn!("database_query", {
...
fn main() {
    ...
    let _interval = logwise_runtime::perfwarn_begin_if!(
...
let _guard = logwise_runtime::heartbeat("critical_task", Duration::from_secs(5));
critical_task();
...
use logwise_runtime::{Level, log_enabled};
...
fn main() {
    ...
    if log_enabled!(Level::Trace) {
        ...
        logwise_runtime::trace_sync!("Debug data: {data}", data=expensive_data);
    }
}
```
...
fn main() {
    ...
    logwise_runtime::mandatory_sync!("Debugging value: {value}", value=42);
}
```
...
fn main() {
    ...
    let _interval = logwise_runtime::profile_begin!("data_processing");
    process_data();
...
fn main() {
    let elapsed_ms = 42;
    logwise_runtime::profile_sync!("Computation took {ms} ms", ms=elapsed_ms);
}
```
...
use logwise_runtime::{context::Context, privacy::LogIt};
...
fn main() {
    ...
    logwise_runtime::info_sync!("Starting application", version="1.0.0");
    ...
    logwise_runtime::info_sync!("Configuration loaded",
        config=LogIt(&config)
    );

    // Track performance-critical operations
    logwise_runtime::perfwarn!("database_connection", {
        // connect_to_database(&config.database_url)
    });

    logwise_runtime::info_sync!("Application ready", port=config.port);
}
```
...
```rust
use logwise_runtime::{Logger, LogRecord};
use std::sync::Arc;
use std::fmt::Debug;
use std::pin::Pin;
...
// Register the logger
logwise_runtime::add_global_logger(Arc::new(CustomLogger));
```
...
[`Context::begin_trace()`]: context::Context::begin_trace
...
pub mod hidden {
    pub use crate::dispatch::{
        ...
        profile_begin_post, profile_begin_pre, profile_sync_post, profile_sync_pre,
...
extern crate self as logwise_runtime;
