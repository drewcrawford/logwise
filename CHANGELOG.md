...
- **Logging from a destructor at thread exit aborted the process.** `Context::current()` reached for its thread-local with `with`, which panics once that thread-local has been destroyed -- and thread-local destructors run in an unspecified order, so anything that logs on drop can easily outlive logwise's context cell. The panic then escaped a thread-local destructor, which is not a catchable failure but `fatal runtime error: thread local panicked on drop, aborting`: the whole process, killed by a log line. `Context::currently_tracing` already took the careful route here; `current`, `set_current`, and `pop` now do too, falling back to a shared process-lived context whose task is never dropped, so a late log still carries a usable prelude instead of taking the process with it.

- **The runtime never forgot a completed span.** Every `span!`, `perfwarn!`, `active_span!`, and `profile!` guard pushed a `CompletedSpan` onto an unbounded `Vec` when it dropped, and the only thing that ever emptied it was an application calling `take_completed_spans()`. An application that never calls it -- which is every application that has not yet wired up a span reader -- grew that `Vec` by one entry per span for the life of the process, with a span guard costing about as much to create as a log call. Retention is now bounded to the most recent `COMPLETED_SPAN_RETENTION` (1024) completions, and `RuntimeDeliveryStats::completed_spans_dropped` reports what a slow or absent reader missed, which is what the in-memory, async, and flight-recorder buffers already did.

- **Exiting tracing spans out of order left a finished span's context entered forever.** `tracing::span::Entered` is an ordinary value, so `drop(outer)` before `drop(inner)` is legal and delivers `exit(outer)` first -- and `LogwiseLayer::on_exit` only popped its guard stack when the exiting span happened to be on top. Anything else was left in place: the guard for `outer` was never dropped, so every later logwise event on that thread was attributed to a span that had already ended, and the thread-local stack grew by one entry each time. The stack now unwinds from the top down through the exiting span, and spans that are still entered are re-entered afterwards so they keep their own context.

- **A field value that was a one-argument call did not compile.** `event!("cache.miss", size = compute(3))` was read as the privacy tag `compute` applied to `3`, because the field muncher matched any `$privacy:ident($value:expr)` and only *then* asked whether the identifier was one of `support`/`local`/`secret`. What the caller got was `no rules expected identifier `compute`` pointing at logwise's own macro internals, with a `while trying to match `support`` note that gives no hint about the actual call site. `Some(x)`, `Wrapper(x)`, and any helper of the shape `f(arg)` were all unusable as field values -- and `f(a, b)`, `T::from(x)` and `f(x) + 1` worked, which made the rule look arbitrary. The three privacy keywords are now matched by name, so anything else falls through to the ordinary value arm and, if the type is not loggable, fails with a `ValueRef: From<_> is not satisfied` error that names the type you passed.

- **A blocking flush barrier could park after the signal it was waiting for.** The async sink's worker published `flushed_sequence` and then notified the barrier condvar without ever touching the lock that barrier waits under. `FlushBarrier::wait` reads that same value and then parks, so a notification landing between the read and the park reached no one, and the barrier slept with nothing left to wake it -- `flush_blocking` and `emergency_drain` hang, typically at shutdown, when they are the last flush of the run. The worker now notifies under the barrier lock, which is what makes the barrier's check-then-park indivisible. The window is a handful of instructions wide and we did not manage to drive it into failing, so this is fixed by construction rather than by regression test.
